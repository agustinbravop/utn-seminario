...
export class ApiError extends Error {
  ...
  /** Si el status es 409, retorna el `ApiError.message`. Sinó, retorna `defaultMsg`. */
  conflictMsg(defaultMsg = "Ocurrió un error") {
    return this.status === 409 ? `${this.message}.` : defaultMsg;
  }
}
